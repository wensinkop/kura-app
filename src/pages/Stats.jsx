...
function rangeFor(mode, anchor, periodStart, periodEnd) {
  if (mode === 'week') {
    ...
    const yr = s.getFullYear() === last.getFullYear() ? `${last.getFullYear()}` : ''
    return { start: isoOfDate(s), end: isoOfDate(e), label: `${shortDay(s)} – ${shortDay(last)} ${yr}`.trim() }
  }
  if (mode === 'year') {
  ...
  if (mode === 'period') {
    ...
    return { start: periodStart, end: endEx, label: null }
  }
  // month
  const y = anchor.getFullYear()
...
export default function Stats() {
  ...
  const [drill, setDrill] = useState(null) // selected top-level category id (drilldown)
  ...
  // setState is deferred into the timeout callback (not run synchronously in the
  // effect body) to satisfy react-hooks/set-state-in-effect.
  useEffect(() => {
    if (!range) {
      const id = setTimeout(() => { setDrill(null); setTxns([]); setLoading(false) }, 0)
  ...
  const drillGroup = agg.groups.find((g) => g.id === drill) || null
  ...
  return (
    <div className="max-w-[760px] mx-auto">
      ...
      {/* Period range navigation / custom pickers */}
      {mode === 'period' ? (
        ...
        <div className="flex items-center justify-center gap-1 mt-3.5">
          ...
          <span className="font-bold text-[15px] px-3 py-1.5 min-w-[160px] text-center tabular">{range?.label}</span>
      ...
      {mode === 'period' && !range ? (
        ...
        <>
          ...
          {/* Expenses by category — or the drilldown for a tapped category */}
          {drillGroup ? (
            <CategoryDrill group={drillGroup} base={base} rates={rates} catMap={catMap}
              ...
              {agg.groups.length === 0 ? (
                ...
                <div className="bg-surface border border-border rounded-[14px] overflow-hidden">
                  {agg.groups.map((g, i) => {
                    ...
                    return (
                      <button key={g.id} onClick={() => setDrill(g.id)}
...
// Drilldown: sub-category breakdown + every transaction for the tapped top-level
// category in the current period.
function CategoryDrill({ group, base, rates, catMap, onBack, onTx }) {
  // Sub-category buckets (base currency). Direct charges on the parent and
  // uncategorised-within go into their own buckets.
  const subs = new Map()
  for (const t of group.txns) {
    const v = toBase(Number(t.amount) || 0, t.currency, rates, base) || 0
    const c = t.category
    let key, name
    if (c && c.parent_id) { key = c.id; name = c.name }
    else if (c) { key = '__direct'; name = `${c.name} (no sub-category)` }
    else { key = '__none'; name = 'Uncategorised' }
    if (!subs.has(key)) subs.set(key, { key, name, total: 0 })
    subs.get(key).total += v
  }
  const subList = [...subs.values()].sort((a, b) => b.total - a.total)
  const txns = [...group.txns].sort((a, b) => (a.date < b.date ? 1 : a.date > b.date ? -1 : 0))

  return (
    <>
      <button onClick={onBack} className="flex items-center gap-1.5 text-sm font-semibold text-muted mt-5 mb-2 hover:text-text">
      ...
      <div className="bg-surface border border-border rounded-[14px] px-4 py-3.5 flex justify-between items-baseline">
        ...
        <span className="font-extrabold text-[15px] tabular text-expense">{formatMoney(group.total, base)}</span>
      </div>

      {subList.length > 0 && (
        <div className="bg-surface border border-border rounded-[14px] overflow-hidden mt-3">
          {subList.map((s) => {
            const pct = group.total > 0 ? Math.round((s.total / group.total) * 100) : 0
            return (
